/**
 * @(#) Seed.java
 * Copyright 2010-2010 by  Click&Touch Inc.
 * @author JANGHASO
 * @MAKE 2010.11. 20.
 * @EDIT
 */
package com.zslide.ocb.Engine;

import com.zslide.ocb.DataLengthException;
import com.zslide.ocb.NoSetIVParamException;
import com.zslide.ocb.NoSuchModeException;
import com.zslide.ocb.NoSuchPaddingException;

/**
 * SEED 암복호화 Class
 *
 * @comment 외부로 부터 아이디와 패스워드 입력 받아 인증 처리 후 결과값을 리턴해주는 모듈에서 사용하는 암/복호화
 */
public final class Seed {
    ...
    public static int BLOCK_SIZE = 16;            // 암복호화 단위 (Bytes)
    public static int KEY_SIZE = 16;            // key size

    public static String MODE_ECB = "ECB";
    public static String MODE_CBC = "CBC";
    public static String MODE_CFB = "OFB";
    public static String MODE_OFB = "CFB";

    public static String PAD_NO = "NOPadding";
    public static String PAD_PKCS = "PKCSPadding";
    public static String PAD_X923 = "X923Padding";
    ...
    public void init(boolean cryptMode, String modeID, String padType, byte[] key) throws IllegalStateException {
        this.forEncryption = cryptMode;
        this.modeID = modeID;
        this.padType = padType;
        wKey = createWorkingKey(key);

        if (!(modeID.equals(MODE_ECB) || modeID.equals(MODE_CBC) || modeID.equals(MODE_CFB) || modeID.equals(MODE_OFB))) {
            throw new NoSuchModeException("No such mode : " + modeID);
        }

        if (!(padType.equals(PAD_NO) || padType.equals(PAD_PKCS) || padType.equals(PAD_X923))) {
            throw new NoSuchPaddingException("No such Padding type : " + padType);
        }

        if (!modeID.equals(MODE_ECB)) {
            throw new NoSetIVParamException("IV Parameter not set");
        }

        if (key.length != KEY_SIZE) {
            throw new DataLengthException("Crypt key too short or too long");
        }
    }

    public void init(boolean cryptMode, String modeID, String padType, byte[] key, byte[] iv) throws IllegalStateException {
        this.forEncryption = cryptMode;
        this.modeID = modeID;
        this.padType = padType;
        this.iv = iv;
    ...
    private int[] createWorkingKey(byte[] inKey) {
	    ...
	    {
	    	Log.d("key", String.format("%d", key[i]));
	    }
	    */
