...
public class CssTokenizer {
    ...
    /**
     * Current numeric value. Valid for TT_NUMBER,
    ...
    /**
     * Reads a numeric value (e.g. 0.3, .3, 999) and writes the value to
     * nval. If then number is followed by % or an identifier, the unit is read to
     * sval. The token type is setFrom to TT_NUMBER, TT_PERCENT or TT_DIMENSION
     * accordingly.
     */
    private void readNumeric(boolean fraction) {
        float val = 0;
        int div = fraction ? 1 : 0;
        do {
            if (nextChar == '.') {
                div = 1;
            } else {
